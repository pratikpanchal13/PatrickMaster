...
//  almofireDemo
//
//  Created by indianic on 18/01/17.
...
class KPAPIManager: NSObject {
    ...
    /**
     ...
     KPAPIManager.GET(strPostURL, param: nil, controller: self, successBlock: { (jsonResponse) in
    ...
    /**
     ...
     KPAPIManager.POST(strPostURL, param: aDictParam, controller: self, successBlock: { (jsonResponse) in
     print("success response is received")
     }) { (error, isTimeOut) in
    ...
    /**
     ...
     let aDictParameter: [String: Any] = [
                        "email": "kushal.panchal@indianic.com" as Any,
     ...
     KPAPIManager.UPLOAD(strPostURL, param: aDictParameter, controller: self, successBlock: { (jsonResponse) in
     print("success response is received")
     }) { (error, isTimeOut) in
